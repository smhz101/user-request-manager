...
class URM_Public {
    ...
    public function add_frontend_modal_content() {
        ...
        <!-- Floating Button -->
        <button id="floatButton" class="button-styled button-float"><?php _e('Open Modal', 'user-request-manager'); ?></button>
        ...
        <div id="customModal">
            ...
            <form id="userRequestForm">
                <div class="form-group">
                    <label for="userEmail"><?php _e('Email', 'user-request-manager'); ?></label>
                    <input type="email" name="user_email" id="userEmail" class="form-input">
                </div>
                <div class="form-group">
                    <label for="subjectDropdown"><?php _e('Subject', 'user-request-manager'); ?></label>
                    <select name="subject" id="subjectDropdown" class="form-select">
                        <option value="issue1"><?php _e('Issue 1', 'user-request-manager'); ?></option>
                        <!-- @TODO: Later will add dynamically populating the options based on admin settings -->
                        <option value="custom"><?php _e('Custom', 'user-request-manager'); ?></option>
                    </select>
                </div>
                <div class="form-group">
                    <label for="customSubject"><?php _e('Custom Subject', 'user-request-manager'); ?></label>
                    <input type="text" name="custom_subject" id="customSubject" class="form-input">
                </div>
                <div class="form-group">
                    <label for="message"><?php _e('Request', 'user-request-manager'); ?></label>
                    <textarea name="message" id="message" class="form-textarea"></textarea>
                </div>
                <div class="form-group">
                    <button type="submit" class="form-button"><?php _e('Submit', 'user-request-manager'); ?></button>
                </div>
            </form>
            <button id="closeModal"><?php _e('Close', 'user-request-manager'); ?></button>
        </div>
